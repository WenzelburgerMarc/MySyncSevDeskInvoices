...
class MySyncSevDeskInvoices extends Module
{
    ...
    public function install()
    {
        ...
        $this->databaseOperations->installLogSQL();
        $this->databaseOperations->addLog(Configuration::get('MY_SYNC_SEVDESK_API_REQUEST_URL', ''));

        return $this->installTab() && $this->databaseOperations->installExistingSevDeskInvoicesSQL() && parent::install() && $this->registerHook('actionValidateOrder') && $this->registerHook('actionOrderStatusPostUpdate') && $this->registerHook('actionPaymentConfirmation') && $this->databaseOperations->addLog('Module installed');
    }
    ...
    private function createNewCustomerInSevDesk($sevDeskUrl, $sevDeskToken, $params)
    {
        try {
            ...
            $this->databaseOperations->addLog('Error during creating new customer in SevDesk: ' . $e->getMessage());
            return null;
        } finally {
            curl_close($ch);
        }
    ...
    private function createNewInvoiceInSevDesk($sevDeskUrl, $sevDeskToken, $params, $sevDeskContactId)
    {
        try {
            ...
            $this->databaseOperations->addLog('Error during creating new invoice in SevDesk: ' . $e->getMessage());
            return null;
        } finally {
            curl_close($ch);
        }
    ...
    private function bookInvoice($sevDeskUrl, $sevDeskToken, $id, $paymentData)
    {
        try {
            ...
            $this->databaseOperations->addLog('Error during booking a payment for an invoice: ' . $e->getMessage());
            return null;
        } finally {
            curl_close($ch);
        }
    ...
    {
        try {
            $sevDeskInvoiceId = $this->databaseOperations->getSevDeskInvoiceIdByPsOrderId($id_order);
            ...
            $this->databaseOperations->addLog('Error canceling invoice: ' . $e->getMessage());
            return null;
        } finally {
            curl_close($ch);
        }
    ...
    private function getSevDeskCountries($sevDeskUrl, $sevDeskToken)
    {
        try {
            ...
            $this->databaseOperations->addLog('Error in retrieving SevDesk countries: ' . $e->getMessage());
            return null;
        } finally {
            curl_close($ch);
        }
    ...
    private function createNewContactAddressInSevDesk($sevDeskUrl, $sevDeskToken, $sevDeskContactId, $name, $addressLine, $city, $zip, $countryId)
    {
        try {
            ...
            $this->databaseOperations->addLog('Error in creating new contact addresses in SevDesk: ' . $e->getMessage());
            return null;
        } finally {
            curl_close($ch);
        }
    ...
    private function getSevUserId($sevDeskUrl, $sevDeskToken): int
    {
        try {
            ...
            $this->databaseOperations->addLog('Error in getting SevDesk SevUser id: ' . $e->getMessage());
            return -1;
        } finally {
            curl_close($ch);
        }
    ...
    private function getExistingSevDeskPaidAmount($sevDeskInvoiceId)
    {
        try {
            ...
            $this->databaseOperations->addLog('Error during fetching existing SevDesk paid amount: ' . $e->getMessage());
            return 0;
        } finally {
            curl_close($ch);
        }
